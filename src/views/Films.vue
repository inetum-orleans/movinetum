...
const listeFilmsFiltres = computed(() => {
  // Version simple avec un foreach et une variable result que nous remplissons
  if (listeFilms.value === null || listeGenresFiltres.value.length === 0) {
    return listeFilms.value
  }

  const toReturn: ListeFilmsPaginee = JSON.parse(JSON.stringify(listeFilms.value))

  toReturn.results = []
  listeFilms.value.results.forEach(film => {
    let filmContainsAllGenres = true
    listeGenresFiltres.value.forEach(genre => {
      if (!film.genre_ids.includes(genre)) {
        filmContainsAllGenres = false
        return
      }
    })

    if (filmContainsAllGenres) {
      toReturn.results.push(film)
    }
  })
